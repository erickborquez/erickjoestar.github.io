...
class Bomb {
    ...
    get realPos() { return new Vec(Math.floor(this.pos.x), Math.floor(this.pos.y)); }
...
Bomb.prototype.update = function (time, state) {
    ...
    let moved = pos.plus(speed.times(time));

    let touch = state.level.touches(moved, this.size.plus(new Vec(.1,0)));
...
Player.prototype.update = function (time, state, keys) {
    ...
    if (!movedToX) {
        ...
        bombs = state.actors.filter(actor => actor.type == 'bomb');
        for (let bomb of bombs) {
            if (overlap(
                { size: this.size.plus(new Vec(1, 0)), pos: movedX },
                { size: bomb.size.plus(new Vec(1, 0)), pos: bomb.pos })) {
                bomb.speed = new Vec(xSpeed, 0);
                break;
            }
        }
    }
    ...
    if (!movedToY) {
        ...
        for (let bomb of bombs) {
            if (overlap(
                ...
                { size: bomb.size.plus(new Vec(0, 1)), pos: bomb.pos })) {
                bomb.speed = new Vec(0, ySpeed);
                break;
            }
        }
    }
